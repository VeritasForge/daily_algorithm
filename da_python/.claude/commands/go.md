...
LeetCode Example만 넣지 말고, 문제의 제약 조건과 특성을 분석하여 테스트 케이스를 보강한다.
**LeetCode 제공 케이스**와 **보강 케이스**는 반드시 주석으로 구분한다.

**보강 케이스 작성 시 고려사항:**
- **엣지 케이스**: 제약 조건의 경계값 (최솟값/최댓값), 빈 입력, 단일 원소, 최소/최대 크기
- **예외 케이스**: 음수, 0, 중복값, 특수한 구조 등 함정이 될 수 있는 입력
- **추가 검증**: 기본 예제에서 다루지 않은 일반적인 True/False 패턴, 대칭/비대칭 변형 등
- **None/null 위치 변형**: 트리/링크드리스트 문제에서 None 위치에 따른 결과 차이
